...
(define instruction-dialog%
  (class dialog%
    ...
    (super-new [label "Create info.rkt"]
               [width 600]
               [height 480])
    ...
    (define pkg-collection (new radio-box% [label "Package collection:  "]
                                [parent this]
                                [choices '("Use the package name"
    ...
    (define base-libs (map generate-control '("base" "scheme-lib" "compatibility-lib" "threading")))
                           ...
                           [callback (λ (c e)
                                       ...
                                       (content-callback
                                        (string-append "#lang info\n"
                                                       (string-join
                                                        (map (lambda (x)
                                                               (call-with-output-string (λ (p) (write x p))))
                                                             sexps)
                                                        "\n")))
                                       (send this show #f))]))))
